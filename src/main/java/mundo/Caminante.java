...
public class Caminante extends Zombie implements SeMueveEnZigzag {
   ...
   * @param ronda
   */
  public Caminante(int posX, int posY, int direccionX, int direccionY, String estadoActual,
  ...
  public String ataco() {
    if (getEstadoActual().equals(CAMINANDO)) {
      ...
      if (getFrameActual() < 16)
        setFrameActual((byte) (getFrameActual() + 1));
    } else if (getEstadoActual().equals(MURIENDO)
        || getEstadoActual().equals(MURIENDO_INCENDIADO)) {
  ...
  public boolean comprobarDisparo(int x, int y, byte danio) {
    ...
    if (x > posHorizontal + 36 && x < posHorizontal + 118 && y > getPosY() + 5
        && y < getPosY() + 188) {
