...
public class SurvivorCamp implements Cloneable, Comparator<Puntaje> {
  ...
  /**
   * entero incambiable que representa el n�mero de zombies que aparecen en una ronda
   */
  ...
  /**
   * char incambiable que representa el estado del juego sin una partida iniciada
   */
  ...
  /**
   * Zombie que no aparece en el juego pero sirve como nodo para modificar la lista facilmente nodo
   * m�s cercano al personaje (abajo)
   */
  private Zombie zombNodoLejano;
  /**
   * Zombie que no aparece en el juego pero sirve como nodo para modificar la lista facilmente nodo
   * m�s lejano al personaje (arriba)
   */
  private Zombie zombNodoCercano;
  /**
   * Personaje en el campo de batalla que est� disparando
   */
  ...
  private Boss jefe;
  /**
   * estado del juego, puede ser pausado, en curso, sin partida o iniciando ronda
   */
  private char estadoJuego;
  /**
   * ronda en la que se encuentra la partida actual, var�a desde 1 a 10, en la 10 s�lo puede estar
   * el jefe
   */
  private byte rondaActual;
  /**
   * n�mero de zombies que han salido a dar la cara en todo el juego
   */
  private int cantidadZombiesGenerados;
  /**
   * n�mero que representa el arma que se est� mostrando en las especificaciones del arma
   * (panelArmas)
   */
  ...
  /**
   * raiz del arbol binario de puntajes, tiene los mismos datos del arreglo mejoresPuntajes pero
   * est�n ordenados por Score
   */
  ...
  public SurvivorCamp() {
    ...
    rondaActual = 0;
    // son caminantes auxiliares, s�lo necesito las instancias del anterior
    // y el de al frente
  ...
  /**
   * obtiene la ronda en el instante en que se llama el m�todo, 0 si el estado es sin partida
   * 
   * @return ronda actual
   */
  ...
  /**
   * cambia la ronda en la que se encuentra, en general sube una ronda, pero si se carga una partida
   * puede variar
   */
  ...
  /**
   * cambia el personaje que est� disparando, ocurre cuando se carga una partida
   */
   ...
   * 
   * @param posX del disparo
   * @param posY del disparo
  ...
  /**
   * cambia los estados del personaje con corde a que recibe un ara�azo zombie, termina el juego si
   * muere
   */
  ...
  /**
   * Se eliminan todos los zombies que hay en la pantalla, cada uno brinda 50 puntos al personaje
   */
   ...
   * 
   * @throws IOException en caso de que no se haya guardado alg�n puntaje
  ...
  /**
   * carga la �ltima partida guardada devuelve la partida clonada porque la actual pasa a estar sin
   * juego y as� elimina los hilos en ejecuci�n
   * 
   ...
   * 
   * @param carpeta
   * @param personaje para asignarselo a la partida si todos los datos son v�lidos
   * @throws Exception si hay informaci�n inv�lida
   */
  private void cargarDatosCampo(File carpeta, Personaje personaje) throws Exception {
      ...
      if (!lineaActual.startsWith("/") && !lineaActual.equals("")) {
        ...
        if (datos.length > 1) {
          ...
          if (datos.length == 7) {
            ...
            aAgregar = new Caminante(posX, posY, direccionX, direccionY, estadoActual, frameActual,
                salud, ronda);
    ...
    bR.close();
    int zombiesExcedidos = contadorZombiesEnPantalla
        + (personaje.getMatanza() % NUMERO_ZOMBIES_RONDA) - NUMERO_ZOMBIES_RONDA;
  ...
  /**
   * m�todo auxiliar para generar un boss con respecto a la salud que le entra por par�metro
   * 
   * @param ronda
   * @param salud
   */
  ...
  /**
   * m�todo auxiliar que verifica las direcciones a las que se mueven los caminantes la suma de sus
   * direcciones no puede ser menor a 4
   * 
   * @param direccionX
   * @param direccionY
   * @throws DatosErroneosException
   */
  private void verificarDatosCaminante(int direccionX, int direccionY)
      throws DatosErroneosException {
  ...
  /**
   * verifica que los datos generales de loz zombies est�n dentro de los l�mites del juego
   * 
   * @param posX
   * @param posY
   * @param estadoActual
  ...
  private void verificarDatosZombie(int posX, int posY, String estadoActual, byte frameActual)
    ...
    if (posX > ANCHO_PANTALLA - Zombie.ANCHO_IMAGEN || posX < 0 || posY > Zombie.POS_ATAQUE
        ...
        || (!estadoActual.equals(Zombie.CAMINANDO)
            && !estadoActual.equals(Zombie.MURIENDO_INCENDIADO)
   ...
   * 
   * @throws IOException en caso de que el jugador abra el ejecutable desde una carpeta inv�lida
   */
   ...
   * @param datos
   * @param actual
   * @return el texto con la informaci�n de los zombies
   */
  ...
  /**
   * verifica las posiciones de los zombies cercanos y su estado para saber si puede acuchillar
   * 
   * @param x
   * @param y
  ...
  public boolean acuchilla(int x, int y) {
    ...
    if (jefe != null) {
      if (jefe.getEstadoActual().equals(Enemigo.ATACANDO)
          && jefe.comprobarDisparo(x, y, Cuchillo.DANIO)) {
        personaje.setEnsangrentado(false);
        personaje.getCuchillo().setEstado(Arma.CARGANDO);
        seEncontro = true;
        if (jefe.getEstadoActual().equals(Boss.DERROTADO)) {
  ...
  /**
   * devuelve el n�mero de referencia al arma que se encuentra a la derecha de la actual
   * 
  ...
  /**
   * devuelve el n�mero de referencia al arma que se encuentra a la izquierda de la actual
   * 
   * @return n�mero del arma Mostrada
   */
   ...
   * 
   * @param nombreJugador
   * @throws IOException en caso de que ocurra un problema al guardar el puntaje serializado
   */
  public void aniadirMejoresPuntajes(String nombreJugador) throws IOException {
    Puntaje score = new Puntaje(personaje.getScore(), personaje.getHeadShots(),
        personaje.getMatanza(), nombreJugador);
    if (raizPuntajes != null)
  ...
  /**
   * guarda el Puntaje ra�z en la carpeta
   * 
   * @throws IOException en caso de que ocurra un problema al guardar la ra�z con las nuevas
   *         asociaciones
   */
    ...
    for (int i = 0; i < mejoresPuntajes.size(); i++) {
      ...
      for (int j = i; j < mejoresPuntajes.size() - 1; j++) {
        if (masHeadShot.getTirosALaCabeza() - mejoresPuntajes.get(j + 1).getTirosALaCabeza() < 0) {
          ...
          posACambiar = j + 1;
        } else if (masHeadShot.getTirosALaCabeza()
            - mejoresPuntajes.get(j + 1).getTirosALaCabeza() == 0) {
